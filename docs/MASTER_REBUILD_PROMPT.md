...
This rebuild is **new repo + carry over ONE clean asset**. Everything else in
this document is *guidance to follow*, not code to copy. Be precise about the
difference — an AI session or new developer must not assume the whole old
repo is a starting point.

**Source repo to fetch from:**
`https://github.com/abdelrahman787/LASP` — branch **`claude/peaceful-cannon-ktbz8d`**
(the default `main` branch on this repo is an empty initial commit; all real
history lives on that branch).

**Copy verbatim, unmodified, into the new repo — this is the ONLY code that
transfers:**
...
That's the entire pure-Dart engine: `lib/recitation/*` (matching engine,
...
repositories/aggregation), its own `pubspec.yaml` (name
`quran_tasmee3_core`, version `0.9.0`, zero Flutter/platform deps), and its
`test/` directory (120+ passing tests). Fetch it with, e.g.:
...
Do not cherry-pick individual files out of this package and do not "clean it
up" while copying — it is frozen and tested as a unit. If you need a new
capability inside it later, add it as a new pure-Dart module alongside the
existing ones (as `alignment.dart`/`fittingAlign` was added historically),
never by editing the tested matching/scheduler logic in place.

**Everything else in the old repo is reference material only — read it in
the old repo if you need to double-check a detail, but do NOT copy-paste it
into the new repo:**

| Old repo path | Why it's not copied |
|---|---|
| `lib/app/data/streaming_asr_service.dart` (and sibling ASR service files) | Superseded by §4 of this document. Rewrite from the described architecture — the old file also carries dead code paths (`sherpa_onnx_asr_service.dart`, `tarteel_asr_service.dart`) that only exist as stubs to keep the old repo compiling after a dependency removal; there's no reason to bring that debt into a clean repo. |
| `lib/features/**`, `lib/app/app.dart`, `lib/main.dart`, providers, theme | Rebuilt fresh per Phase 3 (§9). The old UI layer was iterated on top of an app shell that predates several architecture pivots; a clean repo should wire Riverpod/providers directly against the final ASR design in §4, not against the old provider file's now-stubbed swap points. |
| `android/`, `ios/`, `pubspec.yaml` (app-level) | Regenerated by `flutter create`, then the Gradle fixes in §5 are applied proactively — don't copy the old `build.gradle.kts` files verbatim; they carry now-unnecessary artifacts (e.g. a removed `packagingOptions.pickFirst` block) mixed in with the parts you do need (the `gradle.afterProject` compileSdk fix, the JVM heap sizing). |
| `docs/`, `CLAUDE.md`, `README.md` | Historical record. This document already extracted everything load-bearing from them. |
| `tools/`, `worker/` (Cloudflare Worker ASR proxy) | Out of scope — the Worker-based cloud ASR path was rejected (§3.1) and is not part of the locked architecture. |
| Model binary assets (`assets/models/**/*.onnx`, `tokens.txt`, fonts) | Not committed to git in the old repo either (gitignored) — re-download from the HuggingFace source named in §2.1 and re-run the same asset prep (append the CTC blank to `tokens.txt`, extract CMVN stats) described in §3.4 and §4.1. |

If in doubt about a specific file: **the only directory you `cp -r` is
`packages/quran_tasmee3_core`.** Everything else you write new, informed by
the reasoning in this document.
